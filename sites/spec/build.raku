...
sub render-dashboard(%site, %by-cat --> Str) {
    ...
    <h2 class="conf-areas-title" id="regression-guard">Regression guard <span>&mdash; each kernel’s recorded baseline, in ms, over time</span> <a class="anchor" href="#regression-guard" aria-label="link">#</a></h2>
    <p class="dash-note">The section above asks how Raku++ compares to other engines. This one asks
      a narrower question it has to answer before a release ships: is Raku++ faster
      than Raku++ was? Every point is a baseline recorded by
      <a href="https://github.com/ash/rakupp/blob/main/tools/perf-guard.raku">perf-guard</a>
      into <a href="https://github.com/ash/rakupp/blob/main/tools/perf-baseline.raku">perf-baseline.raku</a>,
      which a release is then gated against; the file’s own git history is the series.
      A kernel’s line starts where that kernel was added, and each was added because a
      regression got through where nothing was watching. All readings come from the one
      machine the baseline file names, so the millisecond values are comparable end to end.
      One recording, on 2026-08-21, was taken on another machine and put back the next
      commit; it is charted as a gap rather than as a reading.</p>
    <div class="dash-bench" id="dash-guard"></div>
