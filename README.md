...
of drills and the Raku++ specification, all on one origin and all running the
...
www/              what gets published, verbatim
  ...
  tour/ spec/ theme/    generated by build.sh — do not edit by hand
...
the tour and spec generators are written in Raku. It copies `theme/` into
`www/theme/`, runs each generator into its path, stamps the `?v=` cache tag,
and then runs five checks — any of which fails the build:
...
The generated output under `www/tour` and `www/spec` **is committed**, so the
